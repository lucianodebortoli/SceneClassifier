...
public class MainActivity extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    /***
     * // Previous implementation using Asynctask:
    ...
    private class RecordTask extends AsyncTask<Void, Integer, float[]> {
        ...
        protected float[] doInBackground(Void... voids) {
            ...
            if (recorder.getState() == 0) {
                Log.d(LOG_TAG, " ERROR : AudioRecord uninitialized");
            } else if (recorder.getState() == 1) {
                Log.d(LOG_TAG, "REC: Recording!...");
                        ...
                        for (int i = 0; i < sizeInFloats; i++) {
                            ...
                            if (sampleIndex >= totalSamples) { // enough samples!
                                ...
                                if (recorder.getState() == 1) {
                                    ...
                                    recorder = null;
                                    isRecording = false;
                                    Log.d(LOG_TAG, "REC: Recording stopped!");
                                    break;}}
                            sampleIndex += 1;}}}
...
//                targetAudio = normalize(targetAudio);}
            ...
            Log.d(LOG_TAG, "Signal Min:"+minimum(targetAudio));
            Log.d(LOG_TAG, "Signal Max:"+maximum(targetAudio));
            Log.d(LOG_TAG, "Signal Average:"+average(targetAudio));
            Log.d(LOG_TAG, "REC: total duration: "+totalDuration+" s");
            return targetAudio;
        }
