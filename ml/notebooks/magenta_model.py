# Copy of audio/ml/models/features_tflite.py
...
def _naive_rdft(signal_tensor, fft_length, padding='center'):
    ...
    if padding == 'center':
        ...
        padded_signal = signal_tensor
    
    result_real_part = tf.matmul(padded_signal, real_dft_tensor, name="real_dft")
    result_imag_part = tf.matmul(padded_signal, imag_dft_tensor, name="imag_dft")
...
def _stft_tflite(signal, frame_length, frame_step, fft_length):
    ...
    window = tf.reshape(tf.constant(
            (0.5 - 0.5 * np.cos(2 * np.pi * np.arange(0, 1.0, 1.0 / frame_length))
            ).astype(np.float32), name='window'), [1, frame_length])
    ...
    real_spectrogram, imag_spectrogram = _naive_rdft(framed_signal, fft_length, padding="right")
...
def waveform_to_mel_spectrogram(x, sample_rate, win_hop, win_length, fft_size, num_mel_bins, lower_edge_hertz=0, upper_edge_hertz=8000, num_spectrogram_bins=None):
...
def waveform_to_melspectrum(win_size, fft_size, mels):
    # Spectrum
    tflite_batchsize=1
    input_shape=(win_size,)
    signal = tf.keras.layers.Input(batch_shape=(tflite_batchsize, ) + tuple(input_shape), dtype='float32', name='input')
    window = tf.signal.hann_window(win_size, name="hanning")
    signal_windowed = signal * window
    real_fft, imag_fft = _naive_rdft(signal_windowed, fft_length=fft_size, padding="right")
    spectrum = tf.sqrt(tf.add(real_fft * real_fft, imag_fft * imag_fft), name='magnitude_spectrum')
    
    # Mel Scale
    mel_weights = tf.signal.linear_to_mel_weight_matrix(num_mel_bins=mels,
                                                       num_spectrogram_bins = fft_size//2+1,
                                                       sample_rate=16000,
                                                       lower_edge_hertz=80,
                                                       upper_edge_hertz=8000)
    melspectrum = tf.matmul(spectrum, mel_weights, name="melspectrum")
    logmelspectrum = tf.math.log(melspectrum + 1e-16, name="log_scale")
    
    # Build Model
    return tf.keras.Model(inputs=signal, outputs=logmelspectrum)

def fill_spectrogram(signal, model, hop_size=160, win_size=320, fft_size=512):
    print("Signal length: ", len(signal))
    logmelspectrogram = []
    loop = True
    n = 0
    while loop:
        chunk = signal[n:n+win_size]
        chunk = adapt_input(chunk)
        #print("chunk shape: ", np.shape(chunk))
        melspectrum = invoke(model, chunk)
        logmelspectrogram.append(melspectrum)
        n += hop_size
        if n > (sr*2 - win_size):
            loop=False
    print("Spectrogram shape: ", np.shape(logmelspectrogram))
